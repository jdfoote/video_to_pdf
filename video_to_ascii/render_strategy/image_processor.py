...
def brightness_to_ascii(i, density=0, reverse = False):
    """
    Get an apropiate char of brighnes from a rgb color
    """
    chars_collection = DENSITY[density]
    if reverse:
        chars_collection = chars_collection[::-1]
...
def pixel_to_ascii(pixel, colored=True, density=0, reverse = False):
    """
    ...
    if colored:
        ...
        char = brightness_to_ascii(bright, density, reverse)
        char = char*2
    return char
